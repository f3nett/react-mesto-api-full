...
function App() {
  ...
  const [currentUser, setCurrentUser] = React.useState({_id: null, name: '', about: '', avatar: ''});
  ...
  function handleUpdateUser(currentUser) {
    api.setUserData(currentUser)
      .then((newUserData) => {
        ...
        console.log('Информация о пользователе обновлена:', {name: newUserData.name, about: newUserData.about});
        closeAllPopups();
      })
      .catch(err => {
        console.log(err);
